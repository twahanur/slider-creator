...
const showImages = (images) => {
  ...
  images.forEach(image => {
    ...
    div.className = 'col-lg-2 col-md-3 col-xs-6 img-item m-2 position-relative box';
    div.innerHTML = `
                    <div class="imgBx">
                    <img class="img-fluid img-thumbnail text-light hover"  onclick=selectItem(event,"${image.webformatURL}") src="${image.webformatURL}" alt="${image.tags}">
                    </div>
                    <div class="content">
                          <p class = "text-center"><span class = "text-capitalize"> ${image.tags}</span></P>
                          <a target="_blank" class='btn btn-success text-white d-flex justify-content-center' href="${image.webformatURL}"  >View Full Screen</a>
                    </div>
...
const selectItem = (event, img) => {
  ...
  element.classList.toggle('added', checkIcon );
...
const createSlider = () => {
  ...
  prevNext.className = "prev-next d-flex w-100 justify-content-between align-items-center bg-transparent";
  prevNext.innerHTML = ` 
  ...
  if (duration > 0) {
    ...
    sliders.forEach(slide => {
      ...
      item.innerHTML = `<img class="w-100" style="width:95%;"
      src="${slide}"
      alt="">`;
